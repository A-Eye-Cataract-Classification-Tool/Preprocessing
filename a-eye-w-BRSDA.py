...
def get_albumentations_transform() -> A.Compose:
    """Returns the standard Albumentations pipeline for post-BRSDA processing."""
    return A.Compose([
        A.HorizontalFlip(p=0.5),
        A.ShiftScaleRotate(shift_limit=0.06, scale_limit=0.06, rotate_limit=20, p=0.5, border_mode=cv2.BORDER_CONSTANT, value=0),
        # Replace RandomBrightnessContrast with ColorJitter
        A.ColorJitter(
            brightness=0.2, 
            contrast=0.2, 
            saturation=0.2, 
...
"""
    ...
    for image_file in tqdm(image_files, desc="Augmenting Images"):
        ...
        for i, choice_idx in enumerate(choices):
            ...
            cv2.imwrite(str(output_path / out_filename), final_img_rgba)
